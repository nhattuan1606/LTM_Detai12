...
typedef struct {
	...
	bool state;		// false: don't logged it, true: logged in
} ClientInfo;
...
char *ProcessSetDeviceParam(char nameOfParam[], char valueOfParam[], bool state) {
	char response[BUFF_SIZE];
	if (state) {
		if (strcmp(nameOfParam, "Nmin") == 0) {
			if (!CheckValid(valueOfParam)) {
				strcpy_s(response, "53");
				return response;
			}
			else {
				machineInfo.Nmin = atof(valueOfParam);
				UpdateFileConfig();
				strcpy_s(response, "50");
				return response;
			}
		}
		else if (strcmp(nameOfParam, "Pmin") == 0) {
			if (!CheckValid(valueOfParam)) {
				strcpy_s(response, "53");
				return response;
			}
			else {
				machineInfo.Pmin = atof(valueOfParam);
				UpdateFileConfig();
				strcpy_s(response, "50");
				return response;
			}
		}
		else if (strcmp(nameOfParam, "Kmin") == 0) {
			if (!CheckValid(valueOfParam)) {
				strcpy_s(response, "53");
				return response;
			}
			else {
				machineInfo.Kmin = atof(valueOfParam);
				UpdateFileConfig();
				strcpy_s(response, "50");
				return response;
			}
		}
		else if (strcmp(nameOfParam, "C") == 0) {
			if (!CheckValid(valueOfParam)) {
				strcpy_s(response, "53");
				return response;
			}
			else {
				machineInfo.C = atof(valueOfParam);
				UpdateFileConfig();
				strcpy_s(response, "50");
				return response;
			}
		}
		else if (strcmp(nameOfParam, "V") == 0) {
			if (!CheckValid(valueOfParam)) {
				strcpy_s(response, "53");
				return response;
			}
			else {
				machineInfo.V = atof(valueOfParam);
				UpdateFileConfig();
				strcpy_s(response, "50");
				return response;
			}
...
char *ProcessGetData(char nameOfParam[], bool state) {
	...
	if (state) {	// If client logged in
		if (strcmp(nameOfParam, "Nmin") == 0) {
			snprintf(response, 15, "60 %f", machineInfo.Nmin);
		}
		else if (strcmp(nameOfParam, "Pmin") == 0) {
			snprintf(response, 15, "60 %f", machineInfo.Pmin);
		}
		else if (strcmp(nameOfParam, "Kmin") == 0) {
			snprintf(response, 15, "60 %f", machineInfo.Kmin);
		}
		else if (strcmp(nameOfParam, "C") == 0) {
			snprintf(response, 15, "60 %f", machineInfo.C);
		}
		else if (strcmp(nameOfParam, "V") == 0) {
			snprintf(response, 15, "60 %f", machineInfo.V);
		}
		else if (strcmp(nameOfParam, "N") == 0) {
			snprintf(response, 15, "60 %f", fRand(100));
		}
		else if (strcmp(nameOfParam, "P") == 0) {
			snprintf(response, 15, "60 %f", fRand(100));
		}
		else if (strcmp(nameOfParam, "K") == 0) {
			snprintf(response, 15, "60 %f", fRand(100));
		}
...
void GetMachineInfo() {
	FILE *f = NULL;
	errno_t err = fopen_s(&f, "configBonPhan.txt", "r");
	if (err != 0) {
	...
	fscanf_s(f, "%lf", &machineInfo.Nmin);
	fscanf_s(f, "%lf", &machineInfo.Pmin);
	fscanf_s(f, "%lf", &machineInfo.Kmin);
	fscanf_s(f, "%lf", &machineInfo.C);
	fscanf_s(f, "%lf", &machineInfo.V);
	machineInfo.state = false;

	printf("code: %s\npass: %s\nNmin: %lf\nPmin: %lf\nKmin: %lf\nC: %lf\nV: %lf\n", machineInfo.machineCode, machineInfo.machinePassword, machineInfo.Nmin, machineInfo.Pmin, machineInfo.Kmin, machineInfo.C, machineInfo.V);
...
void UpdateFileConfig() {
	FILE *f = NULL;
	errno_t err = fopen_s(&f, "configTuoiNuoc.txt", "w");

	if (err != 0) {
		printf("Error code %d: Cannot open file config", err);
	}

	fprintf(f, "%s\n%s\n%lf\n%lf\n%lf\n%lf\n%lf", machineInfo.machineCode, machineInfo.machinePassword, machineInfo.Nmin, machineInfo.Pmin, machineInfo.Kmin, machineInfo.C, machineInfo.V);
