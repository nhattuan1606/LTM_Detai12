...
/**
*  @function StartNormally: Process if choose normal start
*/
void StartNormally() {
...
char *ProcessSetDeviceParam(char nameOfParam[], char valueOfParam[], bool state) {
	...
	if (state) {
		if (strcmp(nameOfParam, "Hmax") == 0) {
			...
			else if (atof(valueOfParam) < machineInfo.Hmin) {
				strcpy_s(response, "53");
				return response;
			}
			else {
				machineInfo.Hmax = atof(valueOfParam);
				UpdateFileConfig();
				strcpy_s(response, "50");
				return response;
			}
		}
		else if (strcmp(nameOfParam, "Hmin") == 0) {
			if (!CheckValid(valueOfParam)) {
				strcpy_s(response, "53");
				return response;
			}
			else if (atof(valueOfParam) > machineInfo.Hmax) {
				strcpy_s(response, "53");
				return response;
			}
			else {
				machineInfo.Hmin = atof(valueOfParam);
				UpdateFileConfig();
				strcpy_s(response, "50");
				return response;
			}
...
char *ProcessGetData(char nameOfParam[], bool state) {
	...
	if (state) {	// If client logged in
		if (strcmp(nameOfParam, "H") == 0) {
			snprintf(response, 15, "60 %f", fRand(100));
		}
		else if (strcmp(nameOfParam, "Hmax") == 0) {
			snprintf(response, 15, "60 %f", machineInfo.Hmax);
		}
		else if (strcmp(nameOfParam, "Hmin") == 0) {
			snprintf(response, 15, "60 %f", machineInfo.Hmin);
		}
...
void GetMachineInfo() {
	FILE *f = NULL;
	errno_t err = fopen_s(&f, "configTuoiNuoc.txt", "r");
	if (err != 0) {
	...
	fscanf_s(f, "%lf", &machineInfo.Hmin);
	fscanf_s(f, "%lf", &machineInfo.Hmax);
	machineInfo.state = false;

	printf("code: %s\npass: %s\nHmin: %lf\nHmax: %lf\n", machineInfo.machineCode, machineInfo.machinePassword, machineInfo.Hmin, machineInfo.Hmax);
...
void UpdateFileConfig() {
	...
	fprintf(f, "%s\n%s\n%lf\n%lf", machineInfo.machineCode, machineInfo.machinePassword, machineInfo.Hmin, machineInfo.Hmax);
